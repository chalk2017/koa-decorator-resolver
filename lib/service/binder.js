...
function routeBinder(router, serviceModules, config, hooks) {
    ...
    for (let name in serviceModules) {
        ...
        for (let subName of moduleFuncs) {
            const [method, controller, url] = (0, injector_1.servInjector)(moduleObj, subName, config);
            if (url) {
