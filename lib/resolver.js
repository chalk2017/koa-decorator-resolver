    ...
    declareTables(sequelize, cacheTabs, transition) {
        ...
        if (cacheTabs && cacheTabs.length > 0) {
            ...
            cacheTabs.forEach((tableName) => {
                const model = this.tablesStructure[tableName]({ s: sequelize, o: commonOpt, t: tableName });
                if (transition) {
            ...
            Object.keys(this.tablesStructure).forEach((tableName) => {
                const model = this.tablesStructure[tableName]({ s: sequelize, o: commonOpt, t: tableName });
                if (transition) {
                    tables[tableName] = this.useTransaction(model, transition);
                }
    ...
    Sqlite(option) {
        const decoratorFunc = (target, propertyKey, { configurable, enumerable, value, writable }) => {
            const func = async (...args) => {
                ...
                let res;
                if (option?.useOrm) {
                    const sequelize = await (0, exports.ormConnectionCreate)(this.sequelize)();
                    let transaction = null;
                    if (useTransaction) {
                        transaction = await sequelize.transaction();
                    }
                    const tables = this.declareTables(sequelize, option.tables, transaction);
                    ...
                    const conn = await (0, exports.connectionCreate)(this.sqlite, this.sqlite3)();
                    target.db = conn;
                    res = await value.apply(target, args);
                    if (autoClose) {
                        await conn.close();
